...
use Drupal\Tests\package_manager\Kernel\TestSandboxManager;
...
END;
  ...
  public static function providerUpdateStageCalled(): array {
    $fixture_dir = static::getDrupalRoot() . '/core/modules/package_manager/tests/fixtures/release-history';
    return [
  ...
  public static function providerStageDestroyedOnError(): array {
    return [
      ...
      // @see \Drupal\package_manager\Event\PreOperationSandboxEvent
      // @see \Drupal\package_manager\Stage::dispatch()
      'pre-create validation error' => [
        PreCreateEvent::class,
        SandboxEventException::class,
      ],
  ...
  public function testStageDestroyedOnError(string $event_class, string $exception_class): void {
    ...
    $this->assertTrue($sandbox_manager->isAvailable());
    $this->runConsoleUpdateStage();
  ...
  public function testStageDestroyedIfNotAvailable(): void {
    ...
    $stage_id = $sandbox_manager->create();
    $original_stage_directory = $sandbox_manager->getSandboxDirectory();
    ...
    $listener = function (PostRequireEvent $event) use (&$cron_stage_dir): void {
      ...
      $this->assertSame($event->sandboxManager->getSandboxDirectory(), $cron_stage_dir);
    ...
    $this->expectException(SandboxOwnershipException::class);
    ...
    $sandbox_manager->claim($stage_id);
  }
  ...
  public function testStageNotDestroyedIfApplying(): void {
    ...
    $this->addEventTestListener(function (PreApplyEvent $event) use ($stop_error) {
      ...
      // update stage.
      $this->assertInstanceOf(TestSandboxManager::class, $event->sandboxManager);
      $this->runConsoleUpdateStage();
  ...
  public function testBeginThrowsException(): void {
    $this->expectExceptionMessage(ConsoleUpdateSandboxManager::class . '::begin() cannot be called directly.');
    $this->container->get(ConsoleUpdateSandboxManager::class)
      ->begin(['drupal' => '9.8.1']);
  }
...
END;
  ...
  public function testCronIsLockedDuringUpdate(): void {
    ...
    $lock_checker = function (SandboxEvent $event) use (&$lock_checked_on_events, $lock) {
   ...
   *
   * @return \Drupal\package_manager\Exception\SandboxEventException
   *   A stage exception carrying the given validation results.
   */
  private function createStageEventExceptionFromResults(array $results): SandboxEventException {
    ...
    $event = new class (
      $this->container->get(ConsoleUpdateSandboxManager::class),
    ) extends SandboxValidationEvent {};

    array_walk($results, $event->addResult(...));
    return new SandboxEventException($event);
  }
